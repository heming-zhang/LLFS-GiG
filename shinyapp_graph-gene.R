  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput('node_threshold',
                  'Select the threshold of filtering important genes',
                  min = 0, max = 1.0,
                  value = 0.7),
...
server <- function(input, output) {
  ...
  output$network <- renderPlot({
    ...
    type_path = paste('./analysis/gigtran-gene/', as.character(type), sep='')
    ...
    type_net_node = read.csv('./analysis/gigtran-gene/t2ds_layer_new_average_fold_node_weight_df.csv')
    ...
    edge_width[E(net)$Weight>=edge_threshold2()] = (E(net)$Weight)*(5.0)
    # edge color
    edge_color = rep('gray', ecount(net))
    edge_color[E(net)$Weight>=edge_threshold2()] = 'black'
