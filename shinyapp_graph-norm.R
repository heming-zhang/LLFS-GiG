...
server <- function(input, output) {
  ...
  output$network <- renderPlot({
    ...
    type_path = paste('./analysis/gigtransformer-norm/', as.character(type), sep='')
    ...
    type_net_node = read.csv('./analysis/gigtransformer-norm/t2ds_layer_norm_average_fold_node_weight_df.csv')
