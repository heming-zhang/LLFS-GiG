...
class LoadData():
    ...
    def load_data(self, fold_n):
        ...
        np.save('./data/post_ml_data/train_idx_' + str(fold_n)  + '.npy', train_idx)
        print(train_idx)
        train_label = train_label_df.drop(columns=['node_idx']).to_numpy()
        print(train_label)
        np.save('./data/post_ml_data/train_label_' + str(fold_n)  + '.npy', train_label)
        print(train_label.shape)
        # [train_x]
        x = np.load('./data/post_ml_data/x.npy', allow_pickle=True)
        train_x = x[train_idx]
        np.save('./data/post_ml_data/train_x_' + str(fold_n)  + '.npy', train_x)
        print(train_x.shape)
        # [train_norm_x]
        norm_x = np.load('./data/post_ml_data/norm_x.npy', allow_pickle=True)
        train_norm_x = norm_x[train_idx]
        np.save('./data/post_ml_data/train_norm_x_' + str(fold_n)  + '.npy', train_norm_x)
        ...
        np.save('./data/post_ml_data/test_idx_' + str(fold_n)  + '.npy', test_idx)
        print(test_idx)
        test_label = test_label_df.drop(columns=['node_idx']).to_numpy()
        print(test_label)
        np.save('./data/post_ml_data/test_label_' + str(fold_n)  + '.npy', test_label)
        print(test_label.shape)
        # [test_x]
        test_x = x[test_idx]
        np.save('./data/post_ml_data/test_x_' + str(fold_n)  + '.npy', test_x)
        print(test_x.shape)
        # [test_norm_x]
        test_norm_x = norm_x[test_idx]
        np.save('./data/post_ml_data/test_norm_x_' + str(fold_n)  + '.npy', test_norm_x)
