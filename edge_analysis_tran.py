...
def read_concat_files(model_type, layer_name, fold_n, patient_type):
    ...
    patient_type_fold_n_gene_first_edge_weight_df = patient_type_fold_n_gene_first_edge_weight_df.groupby(['Actual_From', 'Actual_To']).agg({'Weight':'mean', 'Degree_Row':'mean', 'Degree_Col':'mean'}).reset_index()
...
def edge_minmax_nromalization(model_type, patient_type):
    ...
    s = torch.sparse_coo_tensor(edge_index, edge_weight, (4411, 4411))
    s1 = s.to_dense()
