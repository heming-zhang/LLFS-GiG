...
class GeneTransformerConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj,
        ...
        row, col = edge_index
        deg = degree(col, x[0].size(0), dtype=x[0].dtype)
        # import pdb; pdb.set_trace()
        out = self.propagate(edge_index=edge_index, deg=deg, query=query, key=key, value=value,
    ...
    def message(self, edge_index: Tensor, deg: Tensor,  query_i: Tensor, key_j: Tensor, value_j: Tensor,
